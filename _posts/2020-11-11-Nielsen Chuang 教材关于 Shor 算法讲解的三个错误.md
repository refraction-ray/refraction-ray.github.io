...
正解：要想保证求出正确的 $$r$$，要保证每次测量均匀得出的 $$0\leq s\leq r-1$$ 不能和 $$r$$ 约分（互质）。这样的 s 的数目即为 $$\phi(r)$$。考虑到在 $$r\rightarrow \infty$$ 的极限下，我们有 $$\frac{\phi(r)}{r\log\log r} -> \delta$$，因此我们只需要运行 phase estimation $$O(\log\log r)=O(\log L)$$ 次，从每次提取的分数中选取最大的分母即为真实的 $$r$$。这也是 Shor 原始文章[^Shor]给出的方案。考虑到 phase estimation 中复杂度有 $$O(L^3)$$，这将使得整个复杂度略高于该值。
