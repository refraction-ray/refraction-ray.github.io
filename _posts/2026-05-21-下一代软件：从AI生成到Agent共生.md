...
在 Everyday ArXiv 里，`src/` 里的 Python package 是 deterministic kernel；SKILL.md` 是工作流定义；`user_profile/` 是用户态 memory； `agents.md` 是运行规范；`data/raw/arxiv` 和 `data/reports` 是持久化层；Codex 或 Claude Code 是 execution environment/runtime。
