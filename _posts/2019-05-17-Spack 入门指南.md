...
安装软件只需要 `spack install <package> <variants>` 即可，其中 `<variants>` 可选，代表了相应的编译参数。安装完成后，可以通过 `spack find <package>` 来查找对应的 package 是否安装成功，并通过 `spack location -i <package>`，查看软件的安装位置，其通常都在 `$SPACK_ROOT/opt/spack/<arch>/<compiler>/` 文件夹内。这也就意味着，安装的软件并不在系统路径，无法直接调用。

*`spack find` 的用法其实非常强大。除了直接按照 package 的 syntax 进行搜索以外，也可以诸如 `spack find ^openmpi` 来查找安装的所有依赖 openmpi 的软件。又比如`-x` 可以只显示直接安装的软件，而不现实因为依赖需要而默认安装的软件。*
