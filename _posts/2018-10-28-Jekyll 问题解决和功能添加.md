...
原来的 moon 模版只有基于标签的存档页，而没有基于发布时间的存档页。这样的页面，基于 liquid 的语法很容易构建。唯一需要注意的就是 liquid 模版中对于日期的处理。无论是在模版变量 `{{}}` 还是模板控制段 `{% %}` 中，都是以 `post.date|date: "%b"` 的形式来格式化或拿到日期对象的相应属性的。具体存档页面的实现细节可以查看[源文件](https://github.com/refraction-ray/refraction-ray.github.io/blob/master/archives/index.html)。
